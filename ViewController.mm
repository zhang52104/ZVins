...
NSString* basePath;
//bool calibrateing = NO;
bool calibrateing = YES;
bool startCalibrateImgs = NO;
std::vector< Mat > calibrateImgs;
...
- (void)viewDidLoad {
    ...
    {
        basePath = [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) firstObject] stringByAppendingPathComponent:@"calibrates"];
...
- (void)processImage:(cv::Mat&)image
{
    ...
    {
        bool goback = NO;
        if (calibrateing && !startCalibrateImgs)
            goback = YES;
        
        if (!goback && calibrateing){
            m_buf.lock();
            NSString* toUrl = @"";
            while (1) {
                toUrl = [NSString stringWithFormat:@"%@/%lu.jpg", basePath, calibrateImgs.size()];
                printf("calibrate image path : %s \n", toUrl.UTF8String);
                //break;
                if (![[NSFileManager defaultManager] fileExistsAtPath:toUrl]){
                    break;
                }
                Mat img = cv::imread(toUrl.UTF8String);
                calibrateImgs.push_back(img);
                if (calibrateImgs.size() >= ncount)
                    break;
            }
            if (calibrateImgs.size() < ncount){
                Mat mat;
                cv::cvtColor(image, mat, CV_BGRA2RGB);
                cv::imwrite(toUrl.UTF8String, mat);
                calibrateImgs.push_back(mat);
                if (calibrateImgs.size() % 2 == 0)
                    startCalibrateImgs = NO;
            }else{
                startCalibrateImgs = NO;
            }
            m_buf.unlock();
        ...
        //image.at<float>(0,1) = image.at<float>(1,1);640826.27858283336
...
//        else
        ...
        featuretracker.readImage(img_equa, img_with_feature,frame_cnt, good_pts, track_len, img_msg->header, pnp_P, pnp_R, (vins.solver_flag == VINS::NON_LINEAR));
        TE(time_feature);
        ...
        if(ui_main || start_show == false || vins.solver_flag != VINS::NON_LINEAR)  //show image and AR
        {
            ...
            if(DEBUG_MODE)
            {
                cv::flip(lateast_equa, image, -1);//图像翻转或镜像
            }
            else
            {
                cv::flip(image,tmp2,-1);//图像翻转或镜像
                //cv::rotate(tmp2, image, ROTATE_90_CLOCKWISE);
                //image = tmp2;
                if(vins.solver_flag != VINS::NON_LINEAR || !start_show)
...
getMeasurements()
{
    // 将每帧图与前一帧图中间设备的imu数据，及当前帧图片记录为一个对
...
void send_imu(const ImuConstPtr &imu_msg)
{
    ...
    // 此处还不懂，为什么时0.0；是不是在某些情况不应该是0.0，此处是不是可变的
...
/*
 ...
 VINS 线程：该线程将视觉测量和 imu 数据紧密融合，并解决 WINNDOW 中所有帧的姿势、速度、IMU 偏差、3D 特征
   如果最新的帧是关键帧，则将其推入关键帧数据库
 */
-(void)run{
    [_condition lock];
...
-(void)process
{
    ...
    // 等待唤醒，条件为measurements的size>0
    con.wait(lk, [&]
             {
        if (calibrateing)
            return calibrateImgs.size() >= ncount;
    ...
    if (calibrateing){
        ...
        //cv::Size size = cv::Size(8, 6);
        cv::Size size = cv::Size(6, 8);
        cv::Size square = cv::Size(20.1, 20.1);
        ...
        for (int i = 0; i < calibrateImgs.size(); ++i){
            Mat gray;
            cv::cvtColor(calibrateImgs[i], gray, cv::COLOR_RGB2GRAY);
            
            vector< Point2f > corners;
            if (!cv::findChessboardCorners(gray, size, corners, CALIB_CB_ADAPTIVE_THRESH + CALIB_CB_NORMALIZE_IMAGE + CALIB_CB_FAST_CHECK))
                continue;
            
            cv::cornerSubPix(gray, corners, cv::Size(13, 13), cv::Size(-1, -1), criteria);
            imgPoints.push_back(corners);
            printf("calibrate image corners size : %lu \n", corners.size());
            
            cv::drawChessboardCorners(gray, size, corners, true);
            NSString* toUrl = [NSString stringWithFormat:@"%@/%d-.jpg", basePath, i];
            printf("calibrate image path : %s %d %d\n", toUrl.UTF8String,
                   calibrateImgs[i].cols,
                   calibrateImgs[i].rows);
            cv::imwrite(toUrl.UTF8String, gray);
            ...
            for (int k = 0; k < size.height; ++k){
                ...
                for (int j = 0; j < size.width; ++j){
                    ...
                    Point3f realPoint;
                    realPoint.x = k * square.width;
                    realPoint.y = j * square.height;
                    realPoint.z = 0.0;
                    
                    objpts.push_back(realPoint);
                }
            }
        ...
        vector< Mat > rvecs;
        vector< Mat > tvecs;
        cv::calibrateCamera(objPoints, imgPoints, size, cameraMatrix, distCoeffs, rvecs, tvecs, CALIB_FIX_K3);
        //cv::calibrateCamera(objPoints, imgPoints, size, cameraMatrix, distCoeffs, rvecs, tvecs, CALIB_FIX_K3 | CALIB_FIX_S1_S2_S3_S4 | CALIB_RATIONAL_MODEL);
        
        printf("calibrate cameraMatrix : %lf %lf %lf\n", cameraMatrix.at<double>(0,0), cameraMatrix.at<double>(0,1), cameraMatrix.at<double>(0,2));
        ...
        //printf("calibrate distCoeffs : %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf\n", distCoeffs.at<double>(0,0), distCoeffs.at<double>(0,1), distCoeffs.at<double>(0,2), distCoeffs.at<double>(0,3), distCoeffs.at<double>(0,4), distCoeffs.at<double>(0,5), distCoeffs.at<double>(0,6), distCoeffs.at<double>(0,7), distCoeffs.at<double>(0,8), distCoeffs.at<double>(0,9), distCoeffs.at<double>(0,10), distCoeffs.at<double>(0,11), distCoeffs.at<double>(0,12), distCoeffs.at<double>(0,13));
        ...
        for (int i = 0;i < calibrateImgs.size(); i++)
        {
            ...
            projectPoints(objPoints[i], rvecs[i], tvecs[i], cameraMatrix, distCoeffs, image_points_pro);   //通过得到的摄像机内外参数，对角点的空间三维坐标进行重新投影计算
        ...
        cout << "重投影误差3：" << total_err / calibrateImgs.size() << "像素" << endl << endl;
    ...
    for(auto &measurement : measurements)
    {
        ...
        //update feature position for front-end 更新前端特征位置
        if(vins.solver_flag == vins.NON_LINEAR)
        {
            m_depth_feedback.lock();
        ...
        if(imageCacheEnabled)
        {
            //add state into vins buff for alignwith image 添加状态到vins buff与图像对齐
            if(vins.solver_flag == VINS::NON_LINEAR && start_show)
            {
            ...
            if(vins.marginalization_flag == vins.MARGIN_OLD && vins.solver_flag == vins.NON_LINEAR && !image_buf_loop.empty())
            {
                ...
                if(!first_frame && keyframe_freq % LOOP_FREQ == 0)
                {
                    ...
                    if(vins.Headers[WINDOW_SIZE - 2] == image_buf_loop.front().second)
                    {
                         ...
                         ** and save the pointcloud to the keyframe for reprojection search correspondance
                         我们仍然需要保存测量到关键帧(而不是数据库)，以添加与循环旧姿态的连接，并保存点云到关键帧，以重新投影搜索对应
                         */
                ...
                for(int i = 0; i < keyframe_database.size(); i++)
                {
                    ...
                    if(kf->header == vins.Headers[0])
                    {
                        ...
                        //update edge
                        // if loop happens in this frame, update pose graph;如果在这个帧中发生循环，更新姿势图;
                        if (kf->has_loop)
                        {
        ...
        if (loop_check_cnt < global_frame_cnt)
        {
            ...
            if(loop_succ)
            {
                ...
                if(measurements_old_norm.size()>MIN_LOOP_NUM)
                {
                    ...
                    // add loop edge in current frame 在当前帧中添加循环边
...
/*
 ...
 全局姿态图线程:基于vins的相对姿态优化全局姿态图，更新关键帧数据库
 */
...
- (void)imuStartUpdate
{
   ...
   // motionManager.deviceMotionUpdateInterval = 0.01;
                                        ...
                                        withHandler:^(CMAccelerometerData *latestAcc, NSError *error)
    {
        ...
        acc_msg->header = [[NSDate date] timeIntervalSince1970] ;//latestAcc.timestamp;
...
//        }
//
//        //NSTimeInterval header = motion.timestamp;
//        NSTimeInterval header = [[NSDate date] timeIntervalSince1970];
//
//        shared_ptr<IMU_MSG> imu_msg(new IMU_MSG());
//        //记录用户加速计数据
//        imu_msg->header = header;
//        imu_msg->acc << -(motion.userAcceleration.x + motion.gravity.x) * GRAVITY,
//        -(motion.userAcceleration.y + motion.gravity.y) * GRAVITY,
//        -(motion.userAcceleration.z + motion.gravity.z) * GRAVITY;
//        imu_msg->gyr << motion.rotationRate.x,
//        motion.rotationRate.y,
//        motion.rotationRate.z;
//        printf("imu acc update %lf %lf %lf %lf\n", imu_msg->header, imu_msg->acc.x(), imu_msg->acc.y(), imu_msg->acc.z());
//    }];
            // 开启陀螺仪
...
{
    //delete already exist directory first time 第一次删除已存在的目录
    NSError *error;
    ...
    //creat file directory if it does not exist 如果文件目录不存在，则创建文件目录
