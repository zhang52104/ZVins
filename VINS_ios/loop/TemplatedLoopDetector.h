...
/// Generic Loop detector 通用环路检测器
class TemplatedLoopDetector
{
public:
  
  /// Parameters to create a loop detector 参数创建循环检测器
  struct Parameters
  {
    /// Height of expected images 期望图像高度
    int image_rows;
    /// Width of expected images 期望图像宽度
    int image_cols;
    
    // Main loop detector parameters 主回路检测器参数
    
    /// Use normalized similarity score? 使用标准化相似度评分?
    bool use_nss;
    /// Alpha threshold α阈值
    float alpha;
    /// Min consistent matches to pass the temporal check 最小一致匹配通过时间检查
    int k;
    /// Geometrical check 几何检查
    GeometricalCheck geom_check;
    /// If using direct index for geometrical checking, direct index levels 如果使用直接索引进行几何检验，直接索引等级
    int di_levels;
    
    // These are less deciding parameters of the system 这些是系统较小的决定性参数
    
    /// Distance between entries to be consider a match 要匹配的条目之间的距离
    int dislocal;
    /// Max number of results from db queries to consider 要考虑的db查询的最大结果数
    int max_db_results;
    /// Min raw score between current entry and previous one to consider a match  考虑匹配时，当前条目与前一个条目之间的最小原始分数
    float min_nss_factor;
    /// Min number of close matches to consider some of them 最小数量的接近匹配，以考虑其中一些
    int min_matches_per_group; 
    /// Max separation between matches to consider them of the same group 最大的分离之间的匹配，以考虑他们的同一组
    int max_intragroup_gap; 
    /// Max separation between groups of matches to consider them consistent 匹配组之间最大的分离，以考虑它们的一致性
    int max_distance_between_groups;
    /// Max separation between two queries to consider them consistent 两个查询之间最大的分离，以考虑它们的一致性
    int max_distance_between_queries; 
  
    // These are for the RANSAC to compute the F 这些是供RANSAC计算F的
    
    /// Min number of inliers when computing a fundamental matrix 计算基本矩阵时内嵌线的最小数目
    int min_Fpoints;
    /// Max number of iterations of RANSAC RANSAC的最大迭代次数
    int max_ransac_iterations;
    /// Success probability of RANSAC RANSAC的成功概率
    double ransac_probability;
    /// Max reprojection error of fundamental matrices 基本矩阵的最大重投影误差
    double max_reprojection_error;
    
    // This is to compute correspondences 这就是计算通信
    
    /// Max value of the neighbour-ratio of accepted correspondences 接受的通信的邻域比率的最大值
     ...
     *   expected working frequency (Hz)  根据预期工作频率(Hz)设置部分参数值
     * @param nss use normalized similarity score 使用标准化相似度评分
     * @param _alpha alpha parameter
     * @param _k k parameter (number of temporary consistent matches) K参数(临时一致匹配的数量)
     * @param geom type of geometrical check 几何校验类型
  ...
  private:
    /**
     * Sets some parameters according to the frequency 根据频率设置一些参数
     * @param frequency
     */
...
public:
   ...
   * 使用给定参数和给定词汇表的BoW2数据库创建一个循环检测器
   * @param voc vocabulary 词汇
   * @param params loop detector parameters 循环检测器参数
   */
  ...
  /**
   * Creates a loop detector with a copy of the given database, but clearing
   * its contents 使用给定数据库的副本创建循环检测器，但清除其内容
   * @param T class derived from TemplatedDatabase 从TemplatedDatabase派生的类
   * @param db database to copy
   * @param params loop detector parameters 循环检测器参数
   */
  template<class T>
   ...
   * 检索循环检测器使用的数据库引用
   * @return const reference to database 数据库的const引用
   */
   ...
   * 检索循环检测器使用的词汇表的引用
   * @return const reference to vocabulary  词汇表的const引用
   */
   ...
   * entries are cleared 设置要使用的数据库。清除数据库和检测器的内容
   * @param T class derived from TemplatedDatabase 派生自TemplatedDatabase的类
   * @param db database to copy
   */
  template<class T>
   ...
   * 设置从给定词汇表创建的新DBoW2数据库
   * @param voc vocabulary to copy
   */
  void setVocabulary(const TemplatedVocabulary<TDescriptor, F>& voc);
  
  /**
   * Allocates some memory for the first entries 为第一个条目分配一些内存
   * @param nentries number of expected entries 预期的条目数
   * @param nkeys number of keypoints per image expected 每个图像的关键点数量
   */
   ...
   * 将给定的元组<键、描述符、current_t>添加到数据库中，并返回匹配(如果有的话)
   * @param keys keypoints of the image 图像的关键点
   * @param descriptors descriptors associated to the given keypoints 与给定关键点相关的描述符
   * @param match (out) match or failing information 匹配或失败的信息
   * @return true iff there was match 如果有匹配，返回true
   */
   ...
   * will be 0 again 重置检测器并清除数据库，这样下一个条目将再次为0
   */
  inline void clear();
...
protected:
  ...
  /// Matching island 匹配的岛
  struct tIsland
  {
    /// Island starting entry 岛开始进入
    EntryId first;
    /// Island ending entry 岛结束条目
    EntryId last;
    /// Island score 岛得分
    double score; // score of island
    
    /// Entry of the island with the highest score 以最高分进入本岛
    EntryId best_entry; // id and score of the entry with the highest score
    /// Highest single score in the island 全岛最高的单分
     ...
     * 如果a的入口id小于b的入口id，返回true。假设两个岛屿之间没有重叠
     * @param a
     * @param b
    ...
    /**
     * Returns the length of the island 返回岛屿的长度
     * @return length of island
     */
    inline int length() const { return last - first + 1; }
    
    /**
     * Returns a printable version of the island 返回岛屿的可打印版本
     * @return printable island 可打印的岛屿
     */
  ...
  /// Temporal consistency window 时间一致性窗口
  struct tTemporalWindow
  {
    /// Island matched in the last query Island在最后一个查询中匹配
    tIsland last_matched_island;
    /// Last query id
    EntryId last_query_id;
    /// Number of consistent entries in the window 窗口中一致条目的数量
    int nentries;
    
    /**
     * Creates an empty temporal window 创建一个空的时间窗口
     */
...
protected:
  ...
  /**
   * Removes from q those results whose score is lower than threshold 从q中删除分数低于阈值的结果
   * (that should be alpha * ns_factor) (这应该是alpha * ns_factor)
   * @param q results from query 查询结果
   * @param threshold min value of the resting results 静息结果的最小值
   */
  void removeLowScores(QueryResults &q, double threshold) const;
  
  /**
   * Returns the islands of the given matches in ascending order of entry ids 按条目id升序返回给定匹配的孤岛
   * @param q 
   ...
   * 返回由q的索引在[i_first, i_last](包括这两个)中的条目组成的岛的得分
   * @param q
