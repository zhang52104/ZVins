...
- (void)viewDidLoad {
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:basePath])
        [[NSFileManager defaultManager] removeItemAtPath:basePath error:NULL];
...
int ncount = 40;
long last_stamp = 0;
map<long, cv::Mat> kalibrs;
bool bkalibr = false;
bool bstart = false;
NSString* basePath;
std::vector<std::vector<double> > gyros;
std::vector<long > gyros_stamp;
std::vector<std::vector<double> > acces;
std::vector<long > acces_stamp;

- (void)saveFile{
    
    map<long, cv::Mat>::iterator it = kalibrs.begin();
    for (; it != kalibrs.end(); ++it){
        NSString *toUrl = [NSString stringWithFormat:@"%@/%lu.jpg", basePath, it->first];
        cv::imwrite(toUrl.UTF8String, it->second);
        printf("record play image header : %s\n", toUrl.UTF8String);
    }
}

void interDouble(double v1, double v2, long t1, long t2, double& v3_out, long t3){
    v3_out=v1+(v2-v1)*(t3-t1)/(t2-t1);
}

- (void)processIMU_gyro{
    NSString *documentsDir = [basePath stringByAppendingPathComponent:@"imu0.csv"];
    [[NSFileManager defaultManager] createFileAtPath:documentsDir contents:nil attributes:nil];

    NSString *strCotent = @"";//[NSString stringWithFormat:@"(timestamps=[ns], omega=[rad/s], alpha=[m/s^2])\ntimestamp,omega_x,omega_y,omega_z,alpha_x,alpha_y,alpha_z "];
    
    for(int i = 0; i < gyros.size(); i++){
        for(int j = 0 ; j < acces.size()-1; j++){
            if(gyros_stamp[i] > acces_stamp[j] && gyros_stamp[i] <= acces_stamp[j+1]){
                double x,y,z;
                interDouble(acces[j][0], acces[j+1][0], acces_stamp[j], acces_stamp[j+1], x, gyros_stamp[i]);
                interDouble(acces[j][1], acces[j+1][1], acces_stamp[j], acces_stamp[j+1], y, gyros_stamp[i]);
                interDouble(acces[j][2], acces[j+1][2], acces_stamp[j], acces_stamp[j+1], z, gyros_stamp[i]);
                strCotent = [NSString stringWithFormat:@"%@\n%ld,%lf,%lf,%lf,%lf,%lf,%lf", strCotent, gyros_stamp[i], gyros[i][0], gyros[i][1], gyros[i][2], x, y, z];
                break;
            }
        }
    }
    
    [strCotent writeToFile:documentsDir atomically:YES encoding:NSUTF8StringEncoding error:nil];
}
...
- (void)processImage:(cv::Mat&)image
{
    if(isCapturing == YES)
    {
        if (bkalibr && bstart && kalibrs.size() < ncount){
            long stamp = long([[NSDate date] timeIntervalSince1970] * 1e9);
            if (last_stamp < 1 || stamp - last_stamp > 0.5e9){
                Mat mat;
            ...
            printf("record play image header : %lu\n", kalibrs.size() );
            
            if (kalibrs.size() == ncount){
                [self saveFile];
                bstart = false;
...
//        printf("record play image header : %lf\n", img_msg->header);
//        printf("record play image header : %ld\n", long(img_msg->header * 1e9));
            ...
            if(vins.marginalization_flag == vins.MARGIN_OLD && vins.solver_flag == vins.NON_LINEAR && !image_buf_loop.empty())
            {
                ...
                if(!first_frame && keyframe_freq % LOOP_FREQ == 0)
                {
                    ...
                    if(vins.Headers[WINDOW_SIZE - 2] == image_buf_loop.front().second)
                    {
                        ...
                        printf("adsfasdfasdf database add %f, %f, %f, %f\n", keyframe_database.last_P[0], keyframe_database.last_P[1], keyframe_database.last_P[2], keyframe_database.total_length);
...
-(void)loop_thread{
    ...
    while(![[NSThread currentThread] isCancelled] )
    {
        ...
        if (loop_check_cnt < global_frame_cnt)
        {
            ...
            if (current_image.rows < 1 || current_image.cols < 1){
                if(vins.solver_flag == vins.NON_LINEAR)
                    failImageCount++;
                continue;
            }
...
- (void)imuStartUpdate
{
   ...
   // motionManager.deviceMotionUpdateInterval = 0.01;
                                        ...
                                        withHandler:^(CMAccelerometerData *latestAcc, NSError *error)
    {
        if (bkalibr){
            if ( bstart){
                acces_stamp.push_back(long([[NSDate date] timeIntervalSince1970] * 1e9));
                std::vector<double> imu;
                imu.push_back(-latestAcc.acceleration.x * GRAVITY);
                imu.push_back(-latestAcc.acceleration.y * GRAVITY);
                imu.push_back(-latestAcc.acceleration.z * GRAVITY);
                acces.push_back(imu);
            }
            return;
        }
...
//    }];
    ...
    [motionManager startGyroUpdatesToQueue:[NSOperationQueue currentQueue] withHandler:^(CMGyroData *latestGyro, NSError *error)
    {
        if (bkalibr){
            if ( bstart){
                gyros_stamp.push_back(long([[NSDate date] timeIntervalSince1970] * 1e9));
                std::vector<double> imu;
                imu.push_back(latestGyro.rotationRate.x);
                imu.push_back(latestGyro.rotationRate.y);
                imu.push_back(latestGyro.rotationRate.z);
                gyros.push_back(imu);
            }
            return;
        }
...
- (void)showInputView
{
    ...
    stringView = [NSString stringWithFormat:@"BUG:%d", failImageCount];
