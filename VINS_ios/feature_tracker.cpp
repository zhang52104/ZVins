...
void FeatureTracker::readImage(const cv::Mat &_img, cv::Mat &result, int _frame_cnt, vector<Point2f> &good_pts, vector<double> &track_len, double header, Vector3d &P, Matrix3d &R, bool vins_normal)
{
    ...
    //track
    {
        if(cur_pts.size()>0)
        {
            ...
            //TS(time_track);
            // 计算一个稀疏特征集的光流   有特征由 KLT 稀疏光流算法 [29] 跟踪
            ...
            if(img_cnt!=0)
            {
                for (int i = 0; i< forw_pts.size(); i++)
                {
                   ...
                   // printf("wait for imu read img,  %lf %lf %lf \n", forw_pts[i].x, parallax_cnt[i].min.x, parallax_cnt[i].max.x);
    ...
    if(img_cnt == 0)
    {
        ...
        for(int i = 0; i<ids.size(); i++)
        {
            ...
            double z = 1.0;
            printf(" cut img ptns : %lf, %lf, %lf, %lf \n", cur_pts[i].x, cur_pts[i].y, x, y);
